...
if __name__ =='__main__':
    ...
    if res and res.get('code') == 10000:
        print("----res-----", res)
        # [{'symbol': 'GZ', 'qty': '0', 'freeze_qty': '0', 'otc_freeze_amount': '0'}]
        for bal in res.get('data'):
            if float(bal['qty']) > 0 or float(bal['freeze_qty']) > 0:
                balance[bal['symbol']] = {}
                balance[bal['symbol']]['available'] = float(bal['qty'])
                balance[bal['symbol']]['locked'] = float(bal['freeze_qty'])
    # {'WTA': {'available': 50.6886, 'locked': 0.0}, 'WTC': {'available': 96.1018631637, 'locked': 0.0}}
