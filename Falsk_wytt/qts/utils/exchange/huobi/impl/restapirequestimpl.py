from qts.utils.exchange.huobi.impl import RestApiRequest
from qts.utils.exchange.huobi.impl.utils.apisignaturev2 import create_signature_v2
from qts.utils.exchange.huobi.impl.utils.urlparamsbuilder import UrlParamsBuilder
from qts.utils.exchange.huobi.impl.utils.apisignature import create_signature
from qts.utils.exchange.huobi.impl.accountinfomap import account_info_map
from qts.utils.exchange.huobi.impl.utils.inputchecker import *
from qts.utils.exchange.huobi.impl.utils.timeservice import *
from qts.utils.exchange.huobi.model import *
from qts.utils.exchange.huobi.model.accountledger import AccountLedger
from qts.utils.exchange.huobi.model.deposithistory import DepositHistory, History
from qts.utils.exchange.huobi.model.feerate import FeeRate
from qts.utils.exchange.huobi.model.marketticker import MarketTicker
...
class RestApiRequestImpl(object):
    ...
    def get_best_quote(self, symbol):
        ...
        def parse(json_wrapper):
            ...
            tick = json_wrapper.get_object("tick")
            best_quote.last_price = tick.get_string('close')
    ...
    def get_account_history(self, account_id, currency, transact_types, start_time, end_time, sort, size):
    ...
    def get_trades_list(self, symbol, types, start_time, end_time, from_id, direct, size):
        path = "/v1/order/matchresults"
        builder = UrlParamsBuilder()
        builder.put_url("symbol", symbol)
        builder.put_url("types", types)
        builder.put_url("start-time", start_time)
        builder.put_url("end-time", end_time)
        builder.put_url("from", from_id)
        builder.put_url("direct", direct)
        ...
        def parse(json_wrapper):
            ...
            data_array = json_wrapper.get_array("data")
            for item_in_data in data_array.get_items():
                id = item_in_data.get_int("trade-id")
                order_id = item_in_data.get_int("order-id")
                price = item_in_data.get_string("price")
                amount = item_in_data.get_string("filled-amount")
                fees = item_in_data.get_string("filled-fees")
                currency = item_in_data.get_string("fee-currency")
                symbol = item_in_data.get_string("symbol")
                role = item_in_data.get_string("role")  # maker: 卖  taker: 买
                create_time = item_in_data.get_int("created-at")
                item_info = {
                    "id": id, "symbol": symbol, "orderId": order_id, "price": price, "isBuyer": role,
                    "qty": amount, "commission": fees, "commissionAsset": currency, "time": create_time,
                    }
    ...
    def get_sub_user_list(self, from_id):
        ...
        def parse(json_wrapper):
            ...
            data_array = json_wrapper.get_array("data")
            for item_in_data in data_array.get_items():
                uid = item_in_data.get_int("uid")
                userState = item_in_data.get_string("userState")
                item_info = {"uid": uid, "userState": userState}
                result_list.append(item_info)
            return result_list
