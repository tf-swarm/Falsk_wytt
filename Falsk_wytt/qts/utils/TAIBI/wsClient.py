...
import websocket
from qts.constants import DEPTH, THE_LATEST_DEAL, DEPTHCHART
from qts.utils.TAIBI.conts import WS_DEPTH_TRADE, WS_TEST_DEPTH_SOCKET
    ...
    def on_message(self, msg):
        ...
        if msg:
            ...
            if msg.get('1') and msg.get('3'):
                ...
                if "TAIBI" not in THE_LATEST_DEAL.keys():
                    THE_LATEST_DEAL['TAIBI'] = {}
                if symbol not in THE_LATEST_DEAL['TAIBI'].keys():
                    THE_LATEST_DEAL['TAIBI'][symbol] = {}

                # THE_LATEST_DEAL['TAIBI'] = {}
                # THE_LATEST_DEAL['TAIBI'][symbol] = {}
                THE_LATEST_DEAL['TAIBI'][symbol]['lastTrade'] = {'price': msg.get('4'), '$price': msg.get('5'), '¥price': msg.get('6')}
                THE_LATEST_DEAL['TAIBI'][symbol]['list'] = trade_dict
                THE_LATEST_DEAL['TAIBI'][symbol]['time'] = time.time()
                # print('THE_LATEST_DEAL:', THE_LATEST_DEAL)

                # print('THE_LATEST_DEAL', len(THE_LATEST_DEAL['TAIBI'][symbol]['list']['order']))
            if msg.get('2') and msg.get('3'):
                ...
                if "TAIBI" not in DEPTH.keys():
                    DEPTH['TAIBI'] = {}
                if symbol not in DEPTH['TAIBI'].keys():
                    DEPTH['TAIBI'][symbol] = {}

                DEPTH['TAIBI'][symbol]['asks'] = asks
                DEPTH['TAIBI'][symbol]['bids'] = bids
                DEPTH['TAIBI'][symbol]['ticker'] = ticker
                DEPTH['TAIBI'][symbol]['time'] = time.time() * 1000
                # DEPTH['TAIBI'][symbol]['rate'] = RATE
                # print('深度:DEPTH', id(DEPTH))

                DEPTHCHART['TAIBI'] = {}
                DEPTHCHART['TAIBI'][symbol] = {}
                # sell
                ask_volume = 0
                bid_volume = 0
                asks_list = []
                ...
                DEPTHCHART['TAIBI'][symbol]['asks'] = asks_list
                DEPTHCHART['TAIBI'][symbol]['bids'] = bids_list
                DEPTHCHART['TAIBI'][symbol]['time'] = time.time() * 1000
